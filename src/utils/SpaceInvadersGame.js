class SpaceInvadersGame {
  ...
  setupEventListeners() {
    document.addEventListener("keydown", (e) => {
      ...
      if (
        e.code === "Enter" &&
        (this.gameState === "gameOver" || this.gameState === "win")
      ) {
        e.preventDefault();
        this.restartGame();
      }
  ...
  updatePlayer() {
    ...
    if (this.player.element) {
      this.player.element.setAttribute(
        "transform",
        `translate(${this.player.x - this.player.width / 2}, ${
          this.player.y - this.player.height / 2
        })`
    ...
    for (let i = this.playerBullets.length - 1; i >= 0; i--) {
      ...
      if (bullet.element) {
        bullet.element.setAttribute(
          "transform",
          `translate(${bullet.x - 2}, ${bullet.y - 2})`
        );
      }
  ...
  updateBoss(deltaTime) {
    ...
    if (
      ...
      this.boss.x >= this.gameWidth - this.boss.width / 2
    ) {
      this.boss.moveDirection *= -1;
    }

    if (this.boss.element) {
      this.boss.element.setAttribute(
        "transform",
        `translate(${this.boss.x - this.boss.width / 2}, ${
  ...
  isCollidingWithPlayer(obj) {
    return (
      obj.x < this.player.x + this.player.width / 2 &&
  ...
  addBossHealthBar(parentG) {
    ...
    healthBar.setAttribute(
      "width",
      60 * (this.boss.health / this.boss.maxHealth)
    );
